...
public function login_member($identifier, $pass, $remember = FALSE, $totp_code = NULL, $set_userdata = TRUE) {
		...
		$query = $this->CI->db->get('customers');
...
// print_r($password);die;
		...
		// if not matches
			...
			$res['msg']='';
			$res['error']=$this->CI->lang->line('aauth_error_login_failed_all');
			return $res;
		}
	}
	...
	public function is_allowed($perm_par, $user_id=FALSE){
		...
		var_dump($this->CI->cc_app->eventListen('auth_allowed_permission', [$perm_par])); exit();
			return false;
		}
